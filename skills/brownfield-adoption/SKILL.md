...
|-----------|---------------------|---------------------|
| **Objective** | Add spec coverage around working code | Replace the codebase with a new implementation |
| **What happens to existing code** | Remains in place, evolves under SDD governance | Archived once specs are extracted; new code replaces it |
...
| **Time to first value** | Fast -- specs appear in days, improvements follow | Slow -- significant upfront investment before any return |
...
| **How specs originate** | Derived by analyzing existing behavior | Written forward from product requirements |
| **Handling broken behavior** | Specs capture current state; bugs are fixed through normal SDD cycles | Specs capture intended state; fresh implementation avoids old bugs |
...
### Step 6: Proceed with Normal BPER
