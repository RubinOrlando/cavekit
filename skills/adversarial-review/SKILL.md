...
description: >
  ...
  Deciding Vote, Coverage Audit), how to set up adversarial review with any model via MCP server,
...
| **Delegated Scrutiny** | For substantial review tasks | A dedicated teammate agent manages the full adversarial interaction and delivers a consolidated findings report to the lead |
| **Deciding Vote** | When two approaches conflict | The lead presents both options to the adversarial model, which analyzes trade-offs and recommends a path forward |
| **Coverage Audit** | During the validation phase | Test coverage data and gap analysis are fed to the adversarial model for independent assessment of testing thoroughness |
...
| Delegated Scrutiny | "Own the adversary interaction. Summarize findings for the lead." |
...
Adversarial review fits into the BPER lifecycle at multiple points:

| BPER Phase | Adversarial Role |
|-------------|-----------------|
...
- **prompt-pipeline** -- How to structure builder and reviewer prompts in the BPER pipeline
